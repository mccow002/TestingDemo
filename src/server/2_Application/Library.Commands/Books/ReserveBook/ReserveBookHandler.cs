...
public class ReserveBookHandler(
    ...
    TimeProvider timeProvider) : IRequestHandler<ReserveBookRequest, ReservationViewModel>
{
    public async Task<ReservationViewModel> Handle(ReserveBookRequest request, CancellationToken cancellationToken)
    {
        ...
        var reservation = Reservation.Create(request.BookId, user.UserId, timeProvider);
